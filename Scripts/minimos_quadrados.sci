...
for I=1:N
    ...
    if (I==500) // Adicionando carga nominal
        u(2,1) = Cmn // aqui que muda os parametros
    end 
...
sout = [200, 210, 220, 230, 212]
sins = [1800, 1900, 2000, 2100, 1920]
Y = [];
X = [];
for k = 3:size(sout) %sout = array de dados da saida
    Y = [Y;sout(k)];
    X = [X; -sout(k-1) -sout(k-2) sins(k)]; %sins = array de dados da entrada
end
